            ...
            for tar in cat["target"]:
                if idx == 0: # RSDP
                    environment.append({"P" : ver["P"], "Z": ver["Z"], "LAMBDA" : cat["LAMBDA"], "N" : cat["N"], "K" : cat["K"], \
                            "TNODES" : tar["TNODES"], "SIGSIZE" : tar["SIGSIZE"], "T" : tar["T"], "W" : tar["W"], "CLIB" : tar["CLIB"], \
                            "defines" : tar["defines"]})
                else: # RSDPG
                    environment.append({"P" : ver["P"], "Z": ver["Z"], "LAMBDA" : cat["LAMBDA"], "N" : cat["N"], "K" : cat["K"], \
                            "TNODES" : tar["TNODES"], "SIGSIZE" : tar["SIGSIZE"], "M" : cat["M"], "T" : tar["T"], "W" : tar["W"], \
                            "CLIB" : tar["CLIB"], "defines" : tar["defines"]})

# Creating parameters
params = []
params.append({"KECCAK_UNROLL_FACTOR": str(1), "MAT_DATA_WIDTH": str(64), "DATA_WIDTH": str(64)})
params.append({"KECCAK_UNROLL_FACTOR": str(1), "MAT_DATA_WIDTH": str(192), "DATA_WIDTH": str(64)})
params.append({"KECCAK_UNROLL_FACTOR": str(2), "MAT_DATA_WIDTH": str(192), "DATA_WIDTH": str(64)})
params.append({"KECCAK_UNROLL_FACTOR": str(1), "MAT_DATA_WIDTH": str(712), "DATA_WIDTH": str(64)})
params.append({"KECCAK_UNROLL_FACTOR": str(2), "MAT_DATA_WIDTH": str(712), "DATA_WIDTH": str(64)})
...
def test_top(parameters, env):
    ...
    try:
        run(
            ...
            toplevel="tb_top",  # top level HDL
            module="tb_top",  # name of cocotb test module
