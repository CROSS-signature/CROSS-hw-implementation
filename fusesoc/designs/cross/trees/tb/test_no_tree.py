    ...
    for idx, ver in enumerate(config):
        for cat in ver["category"]:
            for tar in cat["target"]:
                if "FAST" not in tar["defines"]:
                    continue
                if idx == 0: # RSDP
                    environment.append({"P" : ver["P"], "Z": ver["Z"], "LAMBDA" : cat["LAMBDA"], "N" : cat["N"], "K" : cat["K"], \
                        "SIGSIZE" : tar["SIGSIZE"], "T" : tar["T"], "W" : tar["W"], "CLIB" : tar["CLIB"],\
                        "defines" : tar["defines"]})
                else: # RSDPG
                    environment.append({"P" : ver["P"], "Z": ver["Z"], "LAMBDA" : cat["LAMBDA"], "N" : cat["N"], "K" : cat["K"], \
                        "SIGSIZE" : tar["SIGSIZE"], "M" : cat["M"], "T" : tar["T"], "W" : tar["W"], "CLIB" : tar["CLIB"],\
                        "defines" : tar["defines"]})
...
def test_top(parameters, env):
    ...
    try:
        run(
            ...
            toplevel="tb_no_tree",  # top level HDL
            module="tb_no_tree",  # name of cocotb test module
