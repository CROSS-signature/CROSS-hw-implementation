            ...
            for tar in cat["target"]:
                ...
                if idx == 0: # RSDP
                    environment.append({"P" : ver["P"], "Z": ver["Z"], "LAMBDA" : cat["LAMBDA"], "N" : cat["N"], "K" : cat["K"], \
                        "SIGSIZE" : tar["SIGSIZE"], "TREE_NODES_TO_STORE" : tar["TNODES"], "T" : tar["T"], "W" : tar["W"], "CLIB" : tar["CLIB"],\
                        "defines" : tar["defines"]})
                else: # RSDPG
                    environment.append({"P" : ver["P"], "Z": ver["Z"], "LAMBDA" : cat["LAMBDA"], "N" : cat["N"], "K" : cat["K"], \
                        "SIGSIZE" : tar["SIGSIZE"], "TREE_NODES_TO_STORE" : tar["TNODES"], "M" : cat["M"], "T" : tar["T"], "W" : tar["W"], "CLIB" : tar["CLIB"],\
                        "defines" : tar["defines"]})
...
def test_stree_addr(parameters, env):
    tmp = env.copy()
