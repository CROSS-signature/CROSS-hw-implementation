...
@pytest.mark.parametrize("MATRIX", MATRIX, ids=lambda x: pytest_id({"MATRIX": x}))
@pytest.mark.parametrize("VEC_DATA_WIDTH", VEC_DATA_WIDTH, ids=lambda x: pytest_id({"VEC_DATA_WIDTH": x}))
@pytest.mark.parametrize("MAT_DATA_WIDTH", MAT_DATA_WIDTH, ids=lambda x: pytest_id({"MAT_DATA_WIDTH": x}))
@pytest.mark.parametrize("RES_DATA_WIDTH", RES_DATA_WIDTH, ids=lambda x: pytest_id({"RES_DATA_WIDTH": x}))
def test_mul_vector_matrix(VARIANT, CATEGORY, MATRIX, VEC_DATA_WIDTH, MAT_DATA_WIDTH, RES_DATA_WIDTH):
    args = locals()
    sim_build = "sim_build/" + "+".join([str(k) + "-" + str(v) for (k, v) in args.items()])

    if VARIANT == "RSDP" and MATRIX == "M_BAR":
        pytest.skip("Unused CROSS configuration")
    ...
    try:
        run(
            ...
            toplevel="tb_mul_vector_matrix",  # top level HDL
            module="tb_mul_vector_matrix",  # name of cocotb test module
            defines=defines,
